...
(defn draw-line
  ...
  (if width (set! (.-lineWidth ctx) width))
  (if color (set! (.-strokeStyle ctx) color))
  (doto ctx
    (.beginPath)
    (.moveTo sx sy)
    (.lineTo ex ey)
...
(defn draw-seq
  ...
  `:width` and `:color` can be set by those keywords.
  Note: Setting width and color sets for ctx not just this function."
  [ctx sequ & {:keys [width color] }]
  (if width (set! (.-lineWidth ctx) width))
  (if color (set! (.-strokeStyle ctx) color))
  (.beginPath ctx)
  (apply #(.moveTo ctx % %2) (first sequ))
...
(defn draw-rect
  ...
  (if line-width (set! (.-lineWidth ctx) line-width))
  (if color (set! (.-strokeStyle ctx) color))
  (doto ctx
    (.beginPath)
...
(defn draw-arc
  ...
  Note: Setting line-width and color sets for ctx not just this function."
  [ctx x y r a-start a-end & {:keys [lwidth color]}]
  (if lwidth (set! (.-lineWidth ctx) lwidth))
  (if color (set! (.-strokeStyle ctx) color))
  (doto ctx
    (.beginPath)
...
(defn draw-circle
  ...
  line width and color can be set with `:lwidth` and `:color` respectively.
  Note: Setting line-width and color sets for ctx not just this function."
  [ctx x y r & {:keys [lwidth color]}]
  (draw-arc ctx x y r 0 (* 2 Math/PI) :lwidth lwidth :color color)
  )
